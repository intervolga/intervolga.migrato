...
class Iblock extends BaseData
{
	protected function configure()
	{
		...
		$this->setDependencies(array(
			...
			'PRODUCT_IBLOCK_ID' => new Link(IblockIblock::getInstance()),
	...
	public function getList(array $filter = array())
	{
		...
		while ($iblock = $getList->fetch())
		{
			...
			$dependency->setValue(
				IblockIblock::getInstance()->getXmlId(RecordId::createStringId($iblock['IBLOCK_ID']))
			);
			...
			if ($iblock['PRODUCT_IBLOCK_ID'] > 0)
			{
				...
				$dependency->setValue(
					IblockIblock::getInstance()->getXmlId(RecordId::createStringId($iblock['PRODUCT_IBLOCK_ID']))
				);
	...
	protected function recordToArray(Record $record)
	{
		...
		if ($record->getFieldRaw('PRODUCT_IBLOCK_ID') != '0'
			&& $record->getDependency('PRODUCT_IBLOCK_ID')->getValue()
			&& $iblockId = $record->getDependency('PRODUCT_IBLOCK_ID')->findId()
		)
		{
		...
		if ($record->getFieldRaw('SKU_PROPERTY_ID') != '0'
			&& $record->getDependency('SKU_PROPERTY_ID')->getValue()
			&& $propertyId = $record->getDependency('SKU_PROPERTY_ID')->findId()
		)
		{
